...
def train_dBCQ(replay_buffer, num_actions, state_dim, device, parameters,  pol_eval_dataloader, is_demog):
	# For saving files
	...
	while training_iters < parameters["max_timesteps"]:
		for _ in range(int(parameters["eval_freq"])):
			...
			reward_max = torch.max(r)
		

		#evaluations.append(eval_policy(policy, pol_eval_dataloader, parameters["discount"], is_demog, device))  # TODO Run weighted importance sampling with learned policy and behavior policy
		...
		writer.add_scalar('Reward', reward_max, training_iters)
		writer.add_scalar('Q loss', l, training_iters)
